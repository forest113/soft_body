...
#define PM_MASS 0.01       //mass of each vertex
#define SPRING_K 200  //spring constant
...
__global__ void set_impulse_zero_kernel(Point* points, size_t num_points) {
	int i = threadIdx.x;
	if (i < num_points) {
...
__global__ void process_springs_kernel(Spring* springs_list, int size) {
	int i = threadIdx.x;
	if (i < size) {
...
void step(Point* points_list, int num_points, Spring* springs_list, int num_springs) {
	...
	int block_size = 264;
	set_impulse_zero_kernel < < < 1, block_size > > > (points_list, num_points);

	//cout << "impulse:" << points_list[14].impulse.x << " " << points_list[14].impulse.y << " " << points_list[14].impulse.z << endl;
	process_springs_kernel <<<num_springs / block_size, block_size >>>(springs_list, num_springs);
	update_points_kernel <<<num_points / block_size, block_size>>>(TIME_STEP, points_list, num_points);
...
void display()
{
	Point* points_list = G_points_list;
	Spring* springs_list = G_springs_list;
	...
	for (int i = 0; i < num_springs; i++)
	{
		...
		Point* p1 = springs_list[i].p1;
		glVertex3f(p1->pos.x, p1->pos.y, p1->pos.z);
		Point* p2 = springs_list[i].p2;
...
void update(int value)
{
	...
	for (int i = 0; i < (0.001 / TIME_STEP); i++) {
		step(G_points_list, num_points, G_springs_list, num_springs);
	}
